...
To examine how different film characteristics affect the probability of a film receiving a high rating, a series of logistic regression models were fitted. Variables were removed or modified sequentially to evaluate their contribution to model fit.
...
Model 1 includes all main effects and an interaction `(length*budget*log(votes))`, along with `decade` and `genre` as additional categorical predictors. This model aims to capture direct effects and complex relationships between film length, budget, and popularity and genre.

```{r model_1}
model1 <- glm(
  high_rating=="Yes"~length*budget*log(votes)+decade+genre,
  family=binomial(link="logit"),
  data=film)
summary(model1)
```

### Model 2: Removing non-significant interaction

Model 2 simplifies Model 1 by removing the interactions which are not statistically significant, while keeping the interaction `(length*budget)`,and `log(votes)`, `decade`, and `genre`.
...
### Model 3: Dropping decade

Model 3 removes decade as it was not significant in model 2, retaining only the interaction `(length*budget)`, `log(votes)`, and `genre`.

```{r model_3}
model3 <- glm(
  high_rating=="Yes"~length*budget+log(votes)+genre,
  family=binomial(link="logit"),
  data=film)
summary(model3)
```

### Model 4: Dropping budget

Model 4 keeps only the main effect of `length`, the interaction term `length:budget`, `log(votes)`, and `genre`
...
film$high_rating_probability <- predict(model4, type = "response")
